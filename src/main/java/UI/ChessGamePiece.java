...
public abstract class ChessGamePiece {
     ...
     *                           if true, moves will not be generated in the
     *                           constructor
     * @param pieceColor
    ...
    private boolean testMoveForKingSafety(
            ...
            int col) {
        updatePossibleMoves(board);
        ChessGamePiece oldPieceOnOtherSquare = board.getCell(row, col).getPieceOnSquare();
        ChessGameEngine engine = ((ChessPanel) board.getParent()).getGameEngine();
        int oldRow = pieceRow;
    ...
    public boolean isEnemy(ChessGameBoard board, int row, int col) {
        ...
        ChessGamePiece enemyPiece = board.getCell(row, col).getPieceOnSquare() == null
                ? null
                : board.getCell(row, col).getPieceOnSquare();
        if (enemyPiece == null
    ...
    public ArrayList<ChessGamePiece> getCurrentAttackers(ChessGameBoard board) {
        ...
        for (int i = 0; i < board.getCells().length; i++) {
            for (int j = 0; j < board.getCells()[0].length; j++) {
                ChessGamePiece currPiece = board.getCell(i, j).getPieceOnSquare();
                if (currPiece != null
